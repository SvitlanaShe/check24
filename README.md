...
### Check24 login and registration example:

__Note__: All paths relative to directory: app-ui-automation
...
+ To run the tests with a different browser replace the __chromeTest__ above with the following choices:
  ...
  - Chrome Headless :
    * gradlew chromeHeadlessTest -Denv=test
    ...
    ## SearchSpec tests
   For now we have an SocketException, which should be investigated and fixed:
